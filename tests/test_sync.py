...
def test_sync_install(from_line, lines):
    with mock.patch('piptools.sync.check_call') as check_call:
        ...
        check_call.assert_called_once_with(python + ['pip', 'install', '-q'] + sorted(lines))
...
def test_sync_with_editable(from_editable):
    with mock.patch('piptools.sync.check_call') as check_call:
        ...
        if not os.environ.get('VIRTUAL_ENV'):
            python = [sys.executable, '-m']
        check_call.assert_called_once_with(python + ['pip', 'install', '-q', '-e', _get_file_url(path_to_package)])
